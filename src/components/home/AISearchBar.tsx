...
export function AISearchBar() {
  ...
  const handleSubmitQuery = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const filteredSocialEnterprises = socialEnterprises.filter(
        (socialEnterprise) => matchingOutput.includes(socialEnterprise.eid)
      );
  ...
  const handleManualFilter = () => {
    ...
    let filteredDocs =
      userSearchQuery.length !== 0 ? userSearchResults : socialEnterprises;
    ...
    <div className="flex flex-col space-y-8">
      ...
      >
        ...
        <div className="relative lg:w-4/5">
          ...
          />
          {isLoading ? (
            <div className="absolute top-1/2 left-0.9 sm:left-0.7 transform -translate-y-1/2">
              <svg
                aria-hidden="true"
                className="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8  text-gray-400 animate-spin fill-good-goods-blue-900"
                viewBox="0 0 100 101"
                fill="none"
        ...
        {errorMessage.length === 0 ? null : (
          <h6 className="text-red-600 text-sm sm:text-base md:text-lg">
            {errorMessage}
          </h6>
        )}

        <div
          className={`mt-4 text-sm sm:text-base block ${
            userInput.length === maxNumberOfCharacters
              ? "text-red-600"
              : "text-gray-700"
          }`}
        >
      ...
      {/* Suggested prompts */}
      <div
        className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:space-x-4 mt-8"
        id="prompts"
      >
        <div
          className="bg-sky-200 hover:bg-sky-300 px-4 py-2 text-sm md:text-base rounded-full cursor-pointer duration-200 w-fit"
          onClick={() => {
          ...
          <div className="flex flex-col sm:flex-row justify-start items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 w-fit self-start">
            ...
            <div className="text-white font-semibold bg-good-goods-blue-900 hover:bg-sky-700 text-xs sm:text-sm px-3 py-1 rounded-full w-fit flex flex-row items-center justify-center space-x-2">
              ...
              <div
                onClick={() => {
                  setUserSearchQuery("");
                  setUserSearchResults(socialEnterprises);
                  setUserInput("");
                  setDisplay(socialEnterprises);
                  setErrorMessage("");
                  setCurrentPage(1);
                }}
